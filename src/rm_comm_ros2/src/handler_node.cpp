...
class HandlerNode : public rclcpp::Node {
   public:
    HandlerNode() : Node("handler_node") {
        ...
        tx_timer_ = this->create_wall_timer(std::chrono::milliseconds(static_cast<int>(1000.0 / std::max(1.0, hz))),
                                            std::bind(&HandlerNode::publishTxPacket, this));

        if (use_shm_) setupSharedMemory();

        RCLCPP_INFO(this->get_logger(), "handler_node started. use_shm=%s", use_shm_ ? "true" : "false");
    }
   ...
   private:
    ...
    void onRxPacket(const std_msgs::msg::UInt8MultiArray::SharedPtr msg) {
        if (msg->data[0] != 0x72) {
            RCLCPP_WARN(this->get_logger(), "Invalid frame header: expected 0x72, got 0x%02X", msg->data[0]);
            return;
        }
