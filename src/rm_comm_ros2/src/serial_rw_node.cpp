...
class SerialRwNode : public rclcpp::Node {
   public:
    SerialRwNode() : Node("serial_rw_node") {
        ...
        tx_sub_ = this->create_subscription<std_msgs::msg::UInt8MultiArray>(
            "/rm_comm/tx_packet", 10, std::bind(&SerialRwNode::onTxPacket, this, std::placeholders::_1));
        ...
        RCLCPP_INFO(this->get_logger(), "serial_rw_node started. Using port %s, baud %d, use_shm=%s", port_.c_str(),
                    baud_, use_shm_ ? "true" : "false");
    }
   ...
   private:
    void onTxPacket(const std_msgs::msg::UInt8MultiArray::SharedPtr msg) {
        if (msg->data.size() != 64) {
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "tx_packet size %zu != 64, drop",
                                 msg->data.size());
            return;
        }
        ...
        if (fd_ < 0) {
            // 触发重连由读线程负责，此处提示
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "serial not open; drop tx");
            return;
        }
    ...
    void readLoop() {
        while (running_.load()) {
            ...
            if (n > 0) {
                ...
                RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "Head: %d Tail: %d", head, tail);
    ...
    bool openSerial() {
        ...
        if (fd < 0) {
            RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "open %s failed: %s", port_.c_str(),
                                  std::strerror(errno));
            return false;
        }
