...
int main(int argc, char ** argv)
{
  ...
  auto pub_odom_aft_mapped =
    nh->create_publisher<nav_msgs::msg::Odometry>("odom", 1000);
  ...
  while (rclcpp::ok()) {
    ...
    if (sync_packages(Measures)) {
      ...
      if (scan_pub_en || pcd_save_en) publish_frame_world(pub_laser_cloud_full_res, pub_laser_cloud_full_res_dense);
