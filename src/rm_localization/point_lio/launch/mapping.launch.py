...
def _launch_setup(context, *args, **kwargs):
	...
	point_lio_cfg = PathJoinSubstitution([point_lio_dir, "config", "build_map.yaml"]).perform(context)
	point_lio_params = {
	...
	if use_rviz.lower() in ["1", "true", "True", "TRUE"]:
		...
		if config_path is None:
			# fallback to a default rviz under point_lio if available
			candidate = os.path.join(point_lio_dir, "rviz_cfg", "loam_livox.rviz")
			if os.path.exists(candidate):
				config_path = candidate
	...
	actions = [point_lio_node, static_tf_body2base, static_tf_map_init]
