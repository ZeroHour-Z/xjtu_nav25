...
public:
  ...
  , tf_broadcaster_(std::make_unique<tf2_ros::TransformBroadcaster>(*this)) {
    ...
    if (simulate_) {
      RCLCPP_INFO(get_logger(), "Sim mode ON. Meas noise lin=%.3f m/s, ang=%.3f rad/s; Proc noise lin=%.3f m/s, ang=%.3f rad/s (seed=%d)",
    ...
    control_timer_ = create_wall_timer(std::chrono::milliseconds(20), std::bind(&GlobalVelocityControllerNode::onControlTimer, this));

    RCLCPP_INFO(get_logger(), "GlobalVelocityControllerNode started. Expecting target Twist in map frame on %s, and/or Path on %s", target_twist_map_topic.c_str(), path_topic.c_str());
  }
...
private:
  ...
  bool findNearestFreeCell(int start_gx, int start_gy, int &out_gx, int &out_gy) const {
    ...
    while (!q.empty()) {
      ...
      int cur_d = depth_q.front(); depth_q.pop();
  ...
  void onControlTimer() {
    ...
    const bool at_target_cost = have_current_cost && current_cost >= 0 && static_cast<int>(current_cost) <= escape_target_cost_threshold_;

    bool enter_blocked = false;
    if (have_current_cost) {
      if (current_cost < 0) enter_blocked = escape_treat_unknown_as_lethal_;
      else enter_blocked = static_cast<int>(current_cost) >= escape_enter_cost_threshold_;
    }
      ...
      if (worldToGrid(current_x_m, current_y_m, cgx, cgy)) {
        ...
        if (findNearestFreeCell(cgx, cgy, tgx, tgy)) {
          ...
          if (gridToWorld(tgx, tgy, tx, ty)) {
            ...
            RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000, "Entering ESCAPE mode. Target: (%.3f, %.3f) from cost %d", tx, ty, static_cast<int>(current_cost));
          }
        }
      }
    }
    ...
    if (in_escape_mode_) {
      if (at_target_cost) {
        ...
        RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 2000, "Exited ESCAPE mode (reached cost <= %d).", escape_target_cost_threshold_);
      } else {
        ...
        if (dist < escape_goal_tolerance_) {
          ...
          if (worldToGrid(current_x_m, current_y_m, cgx, cgy)) {
            int tgx = 0, tgy = 0;
            if (findNearestFreeCell(cgx, cgy, tgx, tgy) && gridToWorld(tgx, tgy, escape_target_x_m_, escape_target_y_m_)) {
              RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 2000, "ESCAPE re-target to (%.3f, %.3f)", escape_target_x_m_, escape_target_y_m_);
    ...
    if (!have_external) {
      have_internal = planner_.computeTarget(current_x_m, current_y_m, yaw_map_to_base, target_vx_m, target_vy_m, target_wz);
    }
    ...
    double ux_m = kp_xy_ * ex + ki_xy_ * integral_x_ + kd_xy_ * dx_err;
    double uy_m = kp_xy_ * ey + ki_xy_ * integral_y_ + kd_xy_ * dy_err;
