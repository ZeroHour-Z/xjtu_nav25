...
double VelocityPlanner::computeDynamicLookahead(std::size_t start_index) const {
  ...
  while (i + 2 < path_.poses.size() && accumulated_length_m < config_.curvature_window_distance) {
    ...
    const double dx01 = p1.x - p0.x; const double dy01 = p1.y - p0.y;
    const double dx12 = p2.x - p1.x; const double dy12 = p2.y - p1.y;
  ...
  const double mean_abs_curvature = (curvature_samples > 0)
    ? (sum_abs_curvature / static_cast<double>(curvature_samples)) : 0.0;
...
bool VelocityPlanner::computeTarget(double current_x_m, double current_y_m, double current_yaw,
  ...
  if (dist < 1e-6) {
    ...
    const double lookahead_ratio = std::clamp(lookahead_m / std::max(1e-9, config_.lookahead_distance), 0.0, 2.0);
    const double local_speed = std::max(config_.min_target_speed, config_.target_speed * slow_factor * lookahead_ratio);
    const double scale = local_speed / dist;
    target_vx_m = dx * scale;
    target_vy_m = dy * scale;
  }

  const double desired_heading = (dist_to_goal < config_.final_heading_switch_distance) ? goal_yaw : segment_heading;
