...
public:
	explicit PathFollowerNode(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
		...
		action_client_ = rclcpp_action::create_client<nav2_msgs::action::ComputePathToPose>(this, planner_action_name);
		...
		goal_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
			...
			[this](const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
				...
				RCLCPP_INFO(this->get_logger(), "Received new goal from %s: (%.3f, %.3f, %.3f)",
					msg->header.frame_id.c_str(), msg->pose.position.x, msg->pose.position.y, yaw);
		...
		RCLCPP_INFO(get_logger(), "PathFollowerNode started. Publishing target twists on %s", target_twist_topic_.c_str());
	}
...
private:
	...
	void onReplanTimer() {
		if (!action_client_->wait_for_action_server(100ms)) {
			RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000, "Planner action server not available yet");
			return;
		}
		...
		try {
			...
			RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000, "TF lookup for start failed: %s", ex.what());
			return;
		}
		...
		send_goal_options.result_callback = [this](const auto & result) {
			if (result.code == rclcpp_action::ResultCode::SUCCEEDED) {
				...
				RCLCPP_WARN(this->get_logger(), "ComputePath failed (code %d)", static_cast<int>(result.code));
			}
		};
	...
	bool findLookaheadTarget(const geometry_msgs::msg::Pose &base_in_map, geometry_msgs::msg::Point &target_pt, double &path_heading) {
		std::lock_guard<std::mutex> lock(path_mutex_);
		...
		if (target_idx == 0) {
			...
			double dx = current_path_.poses[target_idx].pose.position.x - current_path_.poses[target_idx - 1].pose.position.x;
			double dy = current_path_.poses[target_idx].pose.position.y - current_path_.poses[target_idx - 1].pose.position.y;
	...
	void onControlTimer() {
		...
		// Stop if at goal
		double goal_dist = std::hypot(target.x - base_in_map.position.x, target.y - base_in_map.position.y);
		...
		double yaw_err = std::atan2(std::sin(path_heading - yaw_base), std::cos(path_heading - yaw_base));
